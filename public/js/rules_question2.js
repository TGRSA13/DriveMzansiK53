...
document.addEventListener('DOMContentLoaded', function() {
  ...
  nextButton.addEventListener('click', function(event) {
      ...
      const selectedAnswer = document.querySelector('input[name="rule-question2"]:checked');

      if (selectedAnswer) {
          // Store only the answer letter in localStorage (e.g., 'C', 'B', etc.)
      localStorage.setItem('rule-question2', selectedAnswer.value.charAt(0)); // Save only the letter of the answer

          // Save the remaining time
          localStorage.setItem('quizTimeLeft', timeLeft);

          // Redirect to the next question page
          window.location.href = 'rules_question3.html';  // Replace with your actual next question page
