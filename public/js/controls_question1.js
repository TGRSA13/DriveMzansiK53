...
document.addEventListener('DOMContentLoaded', function() {
  ...
  nextButton.addEventListener('click', function(event) {
    ...
    const selectedAnswer = document.querySelector('input[name="control-question1"]:checked');

    if (selectedAnswer) {
      // Store only the answer letter in localStorage (e.g., 'C', 'B', etc.)
      localStorage.setItem('control-question1', selectedAnswer.value.charAt(0)); // Save only the letter of the answer
